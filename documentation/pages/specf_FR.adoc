...
Le scope du projet englobe l'ensemble des fonctionnalités, des ressources, et des contraintes nécessaires pour la conception, le développement, et le déploiement du site web dédié à la gestion d’afterwork aux différents bars locaux et à la consultation des bières disponibles pour la communauté de l'école. Ce projet comprend les éléments suivants :

=== Catalogue de Bières 

Affichage d'une liste actualisée des bières disponibles dans chaque bar partenaire, avec des détails tels que le type, le degré d'alcool, la brasserie, et les avis des utilisateurs.
...
Interface permettant aux utilisateurs de réserver des tables dans les bars disponibles, avec une gestion des créneaux horaires et des confirmations par email ou notification.

Ce dernier point est une idée plus complete pour un prochain update, pour le moment seul l'option reservation via telephone sera disponible ( à voir ).
...
Définition de l'architecture du système (choix des technologies, structure des bases de données, schéma d'API).
...
Mise en place du serveur, création des bases de données, et développement des API.

Implémentation des fonctionnalités principales comme la gestion des réservations(?), le catalogue de bières, et le système d'authentification(?).
...
Connexion du frontend au backend via les API (si temps et besoin).

Intégration des services externes (Google Maps, notifications(?)).
...
Validation du bon fonctionnement des interactions entre les différentes parties du système (API, base de données, frontend).
...
=== Bars locaux selectionnés
  ...
  - **Propriétaires et Gérants des Bars :**
    ...
    - **Rôle :** Fournir les informations sur les bières disponibles, les horaires d'ouverture, et gérer les réservations reçues via le site(ultérieurement).
...
=== Équipe de Développement
  ...
  - **Chef de Projet :**
    ...
    - **Intérêt :** Assurer que le projet est livré dans les délais, respecte le budget, et répond aux exigences de qualité.
...
=== Fournisseurs de Services Tiers
 ...
 /////////////////// //// //// //// //// IDEE //// //// //// //// //// //// //// //// //// //// ////
  ...
  - **Plateformes de Paiement (si applicable) :**

    - **Rôle :** Gérer les transactions en ligne (par exemple, prépaiement pour des réservations).

    - **Intérêt :** Offrir un service de paiement sécurisé et fiable pour les utilisateurs du site.
...
Intégration avec des services externes tels que Google Maps pour la localisation des bars, les plateformes de paiement (si applicable), et les systèmes de notification. La disponibilité et la performance de ces services sont hors du contrôle direct de l'équipe de développement et peuvent impacter le projet.
...
La dépendance envers les technologies choisies pour le frontend (React, Vue.js), le backend (Node.js, Django), et la base de données (PostgreSQL, MongoDB). Les bugs ou les limitations dans ces technologies peuvent nécessiter des ajustements ou des solutions de contournement.

**Équipe de Développement :**

Disponibilité et compétences de l'équipe de développement. Des absences ou des manques de compétences spécifiques peuvent ralentir le projet ou nécessiter l'embauche de personnel supplémentaire.
...
Les utilisateurs doivent pouvoir réserver des tables dans les bars partenaires via le site, en sélectionnant le bar, la date, l’heure, et le nombre de places.FEATURE à VOIR

Les utilisateurs doivent recevoir une confirmation de leur réservation par email ou notification. FEATURE à VOIR

Possibilité d'appeler pour reserver manuellement.
...
Les utilisateurs doivent pouvoir rechercher un bar ou une bière spécifique en utilisant des critères tels que la localisation, le type de bière, ou le nom du bar.

Les utilisateurs doivent pouvoir filtrer les résultats en fonction de préférences telles que le type de bière, la popularité, ou les avis des utilisateurs.

**Carte Interactive :**

Le site doit intégrer une carte interactive permettant aux utilisateurs de visualiser la localisation des bars partenaires et d'accéder rapidement aux informations les concernant.

**Authentification et Gestion des Comptes :**

Les utilisateurs doivent pouvoir créer un compte, se connecter, et gérer leurs informations personnelles (nom, email, mot de passe, préférences).

Le site doit permettre aux utilisateurs de visualiser et de gérer leurs réservations.

**Système de Notification :**

Le site doit pouvoir envoyer des notifications aux utilisateurs concernant les confirmations de réservation, les changements d’horaires des bars, ou les nouvelles offres disponibles.
...
Le site doit implémenter des mesures de protection contre les attaques courantes (ex. injection SQL, cross-site scripting, etc.).
